  ...
  return fetch(BACKENDURL + "district")
    ...
    .then(function (data) {
      return data.data.map(function (data) {
        return {
          label: `${data.district_name} / ${data.states_name}`,
          value: data.id,
        };
      });
    });
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, full, meta) {
          return `<ul class="list-inline mb-0 text-end">
              <li class="list-inline-item"><a onclick=edit(this) data-id="${data}" data-tehsilid="${full.district_id}" data-item="${full.tehsil_name}" class="avtar avtar-s btn-link-success btn-pc-default"><i class="ti ti-edit f-20"></i></a></li>
...
stateForm.addEventListener("submit", (e) => {
  ...
  var idField = document.getElementById("idField"),
    ...
    stateidField = document.getElementById("stateid");
  if (idField.value > 0) {
    const requestOptions = {
      method: "PUT",
      body: `id=${idField.value}&name=${stateNameField.value}&districtid=${stateidField.value}`,
      headers: {
        Accept: "*/*",
    ...
    fetch(BACKENDURL + "tehsil", requestOptions)
